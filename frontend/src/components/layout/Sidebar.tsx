...
const navLinkClass = ({ isActive }: { isActive: boolean }) =>
  cn(
    'flex items-center gap-3 px-3 py-2.5 rounded-md font-body font-semibold text-sm transition-all',
    isActive
      ? 'bg-green text-white border-2 border-lime/60 shadow-[3px_3px_0_#C7F25C]'
      : 'text-white/65 hover:bg-white/10 hover:text-white border-2 border-transparent'
  )
  ...
  return (
    <aside className="w-60 flex-none flex flex-col bg-ink text-white border-r-2 border-ink">
      ...
      <nav className="flex-1 px-3 py-4 flex flex-col gap-0.5">
        ...
        <p className="px-3 mt-5 mb-2 font-mono text-[9px] font-bold uppercase tracking-widest text-white/30">
          Sistema
        </p>
